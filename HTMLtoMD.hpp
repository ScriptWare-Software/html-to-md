...
namespace html_to_md {
	// Basic HTML tags that we can convert easily
	inline std::unordered_map<std::wstring, std::pair<std::wstring, std::wstring>> basic_markdown_tags = {
		{L"p", {L"\n\n", L""}},
		{L"strong", {L"**", L"**"}},
		{L"b", {L"**", L"**"}},
		{L"em", {L"_", L"_"}},
	...
	*/
	inline std::pair<std::wstring, std::wstring> ParseTag(const std::wstring& tag) {
	...
	*/
	inline std::map<std::wstring, std::wstring> ParseAttributes(const std::wstring& attributes) {
	...
	*/
	inline ASTNode ParseHTMLToAST(const std::wstring& html_input) {
		ASTNode root;
		root.type = ROOT;
		...
		for (int current_index = 0; current_index < html_input.length();) {
			...
			if (c == '<') {
				...
				if (end_tag_index == std::wstring::npos) {
					// Ignore and still create markdown without this specific incomplete part
					std::wcout << L"Warning: Malformed or incomplete HTML input (no closing '>')." << std::endl;
					current_index++;
					continue;
				}
				...
				if (tag.substr(0, 3) == L"!--") { // special case, doesn't have a closing/opening "<"/">".
					...
					if (!is_closing_tag) {
						...
						if (stack.back()->name != tag_name) {
							std::wcout << L"Warning: Closing tag does not match with corresponding opening tag." << std::endl;
						}
						stack.pop_back();
					}
	...
	*/
	inline std::wstring AstToMarkdown(const ASTNode& node, int list_level = 0)
	{
		...
		for (auto& child : node.children)
		{
			...
			case ELEMENT:
				...
				else if (child.name == L"ol" || child.name == L"ul")
				{
					...
					list_level++;
					markdown += L"\n" + AstToMarkdown(child, list_level);
					list_level--;
					counters.pop_back();
				}
				...
				else if (child.name == L"table")
				{
					...
					for (const ASTNode& table_child : child.children)
					{
						if (table_child.name == L"tr")
						{
							...
							for (const ASTNode& cell : table_child.children)
							{
								...
								{
									row_text += L"| " + AstToMarkdown(cell) + L" ";
								}
							}
							row_text += L"|\n";
							body_rows += row_text;
					...
					markdown += L"\n" + caption_text + header_row + separator_row + body_rows + footer_rows + L"\n";
				}
	...
	inline std::wstring ConvertHTMLToMarkdown(const std::wstring& html_input)
	{
